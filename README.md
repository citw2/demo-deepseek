...
**DeepSeek + a memory you own. One client-side-encrypted store. Erasure you can prove.**

> ⭐ **Star this repo and share it** — help every agent get portable, provable memory. [Share on X](https://x.com/intent/tweet?text=DeepSeek%20with%20a%20memory%20you%20own%20-%20portable%2C%20encrypted%2C%20provably%20erasable%20-%20via%20SAIHM.&url=https%3A%2F%2Fgithub.com%2Fcitw2%2Fdemo-deepseek).

A tiny, runnable demo of [SAIHM](https://saihm.coti.global) wired to **DeepSeek**: it stores three personal facts, grounds DeepSeek in them, then **forgets** one fact and shows DeepSeek can no longer use it — provable erasure (GDPR Art. 17). The very same memory is portable to any other model.
...
```
git clone https://github.com/citw2/demo-deepseek
cd demo-deepseek
npm install
node demo.mjs
```

## Use a real DeepSeek answer (BYOK)

With no key set, DeepSeek answers in a deterministic **offline mock**, so the demo runs end to end with zero setup. Set your key to get a real answer:
...
The call is DeepSeek’s standard **OpenAI-compatible** request (`https://api.deepseek.com/chat/completions`, model `deepseek-v4-flash`). Your key is read from the environment and sent only to DeepSeek’s own API. Override the base URL or model id without editing code via `SAIHM_DEEPSEEK_URL` / `SAIHM_DEEPSEEK_MODEL`.

## Why this matters

A per-vendor “memory” feature keeps your context inside one vendor’s account. SAIHM gives you memory that is:

1. **Yours / portable.** One live store grounds DeepSeek *and* every other model (Claude, DeepSeek, Qwen, Kimi, GLM, GPT, a local model, your own agent) — no per-vendor export, no lossy one-time import.
